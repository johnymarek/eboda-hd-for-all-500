...
<channel>
	<title>dailymotion - FOX_Sports_Interactive</title>
	<menu>main menu</menu>
...
//http://www.dailymotion.com/en/visited-today/1
if($page) {
    if($search) {
        $html = file_get_contents("http://www.tube8.com/search.html?q=".$search."&page=".$page);
    } else {
        $html = file_get_contents("http://www.dailymotion.com/user/FOX_Sports_Interactive/".$page."");
    }
} else {
    $page = 1;
    if($search) {
        ...
        $html = file_get_contents("http://www.dailymotion.com/user/FOX_Sports_Interactive/1");
    }
}
...
$videos = explode('<div class="dmpi_video_preview image_border dmpi_video_preview_official">', $html);
