...
if($page) {
	$html = file_get_contents("http://zalaa.com/videos/basic/mr/".$page);
} else {
	$page=1;
	$html = file_get_contents("http://zalaa.com/videos/basic/mr/1");
}
...
foreach($videos as $video) {
    $id = str_between($video,"zalaa.com/video/","/");
    ...
    $pos = strrpos($title, "was uploaded");
    if ($pos !== false) {
    	$title = trim(substr($title,0,$pos));
    }
		$data = "Durata: ".str_between($video,'<span class="duration">','</span>'  );
if ($id <> "") {
		$link = $host.'/scripts/filme/php/zalaa_link.php?file='.$id;
