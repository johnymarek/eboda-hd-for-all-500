...
//http://www.jurnaltv.md/ro/categories_block/3610/news_last.html#0
if($page) {
    $page1 = $page-1;
	$html = file_get_contents("http://www.jurnaltv.md/ro/categories_block/".$search."/news_last.html#".$page1);
} else {
	$page = 1;
  $html = file_get_contents("http://www.jurnaltv.md/ro/categories_block/".$search."/news_last.html#0");
}
...
foreach($videos as $video) {
  $t1 = explode('"', $video);
  $img = $t1[0];
  $image = "http://www.jurnaltv.md".$img;
  $id = str_between($img,"thumbs3_",".jpg");
  $link = "http://video.jurnaltv.md/gallery_video/".$id.".mp4";
  
  $title = str_between($video,'<span class="tc7">','</span>');
  $zi = str_between($video,'<td align="left"><span class="size11">','</span>');
  $ora = str_between($video,'<td align="right"><span class="size11">','</span>');
  $descriere = $zi." - ".$ora;
