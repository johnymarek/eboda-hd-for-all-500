...
class MarocTemplate {
    ...
    public function generateView($category, $title = null) {
        ...
        switch($category) {
            case MarocTemplate::VIEW_HOME;
                $this->getHomeHeader();
                break;
            case MarocTemplate::VIEW_CATEGORY;
                $this->getCategoryHeader();
                break;
            case MarocTemplate::VIEW_PAGE_NUMBERS:
                $this->getPageNumbersHeader();
                break;

            case MarocTemplate::VIEW_MOVIE;
                $this->getMovieHeader();
                break;
            case MarocTemplate::VIEW_MOVIE_DETAIL;
                $this->getMovieDetailHeader();
                break;


            case MarocTemplate::VIEW_PLAY;
                $this->getPlayHeader();
                break;
        }
...
<mediaDisplay name="photoView"
    ...
    private function getCategoryHeader() {
        ?>
<mediaDisplay name="photoView"
              showHeader="yes" rowCount="3" columnCount="5" drawItemText="no" showDefaultInfo="no"
              itemImageXPC="100" itemImageYPC="100" itemOffsetXPC="4" itemOffsetYPC="14" sliding="yes"
              itemWidthPC="17" itemHeightPC="20" itemBorderColor="-1:-1:-1"
              idleImageXPC="90" idleImageYPC="5" idleImageWidthPC="5" idleImageHeightPC="8"
    ...
    <onUserInput>
        <script>
            ...
            if ( userInput == "two" )      {
               ...
               rss = "rss_file:///home/scripts/xVoD/php/scraper/rss/downloadDialog.rss";
               ret = doModalRss(rss);           
               if (ret == "Confirm")    {
