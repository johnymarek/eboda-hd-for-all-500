...
$link="http://sport-mania.sk/worldtv/chooser.html";
$html = file_get_contents($link);
//$videos = explode('"onClick="expandcontent"', $html);
$html=str_between($html,"england special","</table>");
$videos = explode('<a href="', $html);
unset($videos[0]);
$videos = array_values($videos);
foreach($videos as $video) {
  $t1=explode('?',$video);
  $link="http://sport-mania.sk".str_replace("link/","",$t1[0]);
  $t2=explode('>',$t1[1]);
  $t3=explode('<',$t2[1]);
  $title=trim($t3[0]);
  ...
  $t2=explode('"',$t1[1]);
  $img=$t2[0];
  if (strpos($img,"vlcikona.png") !== false) {
    echo '
    <item>
    <title>'.$title.'</title>
    <onClick>
    <script>
    showIdle();
    url="'.$host.'/scripts/tv/php/sport-mania.php?file='.$link.',1";
    movie=getUrl(url);
    cancelIdle();
