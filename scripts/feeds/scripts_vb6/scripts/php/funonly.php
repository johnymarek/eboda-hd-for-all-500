...
if($page) {
    ...
    if($search) {
        ...
        $html = file_get_contents("http://funonly.net/default.aspx/funny_videos_flash_games~".$page."/");
    }
} else {
    $page = 1;
    if($search) {
        ...
        $html = file_get_contents("http://funonly.net/default.aspx/funny_videos_flash_games~1/");
    }
}
...
$videos = explode('<td width="50%" align="left" valign="top">', $html);
...
foreach($videos as $video) {
    ...
    $t2 = explode('"', $t1[1]);
    $link = "http://funonly.net".$t2[0];

    $t1 = explode('src="', $video);
    $t2 = explode('"', $t1[1]);
    $image = "http://funonly.net".$t2[0];
...
//if ($pos1 === false) {
    ...
    $link = urldecode(str_between($html, 'param value="file=', '&'));
//if($link=="") {
