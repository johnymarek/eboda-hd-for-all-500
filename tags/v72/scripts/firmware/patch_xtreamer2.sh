...
IMAGE_FILE1=install.img
IMAGE_FILE2=Xtreamer_2.4.0b20.xtr

[ -f ${IMAGE_FILE1} ] || echo "could not find ${IMAGE_FILE1}" &&  exit
[ -f ${IMAGE_FILE2} ] || echo "could not find ${IMAGE_FILE2}" &&  exit
...
#
# unpack img
mkdir unpacked_install1
[ $? -eq 0 ] || exit cannot create dir unpacked_install1, please start from a clean directory

mkdir unpacked_install2
[ $? -eq 0 ] || exit cannot create dir unpacked_install2, please start from a clean directory

fw_root=`pwd`

cd ${fw_root}/unpacked_install1/
tar xvf ../${IMAGE_FILE1}

cd ${fw_root}/unpacked_install2/
tar xvf ../${IMAGE_FILE2}


#
# go to package in our firmware
cd ${fw_root}/unpacked_install1/package2/

#
# unpack root from xtreamer fw
mkdir unpacked_root
[ $? -eq 0 ] || exit cannot create dir please start from a clean directory

cd unpacked_root/
unyaffs ${fw_root}/unpacked_install2/package2/yaffs2_1.img
...
mkdir -p ../../../yapps/usr_local_bin
mv usr/local/bin/* ../../../yapps/usr_local_bin/
...
mv  ${dir}/ewcp.zip ../../../yapps
mv  ${dir}/ewcp-version.txt ../../../yapps
...
#on yapps for xtreamer
mv ${dir}/cb3pp.zip ../../../yapps
mv ${dir}/cb3pp-version.txt ../../../yapps
...
#on zapps for xtreamer
mv ${dir}/scripts.zip ../../../yapps
mv ${dir}/scripts-version.txt ../../../yapps

#touch our USB marker
echo "cipibad market xtr4all" >  ../../../yapps/.do_not_delete
...
cd unpacked_etc/
unyaffs ${fw_root}/unpacked_install2/package2/yaffs2_2.img
...
USB_ROOT=""
USB_ROOT="/tmp/usb_root"

# check if .../cb3pp installed from us, if not, unpack
SERIAL=0
 [ -f ${storage}/cb3pp-version.txt ] && . ${storage}/cb3pp-version.txt
DISK_SERIAL=${SERIAL}
[ -f ${USB_ROOT}/cb3pp-version.txt ] && . ${USB_ROOT}/cb3pp-version.txt

if [ ${SERIAL} -gt ${DISK_SERIAL} ]
then
	rm -rf  ${storage}/cb3pp/*
	cd ${storage}
	unzip -o ${USB_ROOT}/cb3pp.zip 
	cp ${USB_ROOT}/cb3pp-version.txt ${storage}/cb3pp-version.txt
...
DISK_SERIAL=${SERIAL}
 [ -f ${USB_ROOT}/scripts-version.txt ] && . ${USB_ROOT}/scripts-version.txt

if [ ${SERIAL} -gt ${DISK_SERIAL} ]
then
        rm -rf ${storage}/scripts/*
        cd ${storage}
        unzip -o ${USB_ROOT}/scripts.zip
	cp ${USB_ROOT}/scripts-version.txt ${storage}/scripts-version.txt
fi
...
DISK_SERIAL=${SERIAL}
 [ -f ${USB_ROOT}/ewcp-version.txt ] && . ${USB_ROOT}/ewcp-version.txt

if [ ${SERIAL} -gt ${DISK_SERIAL} ]
then
        rm -rf ${storage}/ewcp/*
        cd  ${storage}
        unzip -o ${USB_ROOT}/ewcp.zip
	cp ${USB_ROOT}/ewcp-version.txt ${storage}/ewcp-version.txt
