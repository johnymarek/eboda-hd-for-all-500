...
if($page) {
    if($search) {
        ...
        $html = file_get_contents("http://www.youtube.com/movies?s=mvp&t=&cr=CZ&p=".$page."");
    }
} else {
    $page = 1;
    if($search) {
        ...
        $html = file_get_contents("http://www.youtube.com/movies?s=mvp&t=&cr=CZ&p=1");
    }
}
...
$videos = explode('<div class="movie-entry', $html);
...
foreach($videos as $video) {
    ...
    $link = 'http://www.youtube.com'.$t2[0]."&hd=1";
    	...
    	$image = $t2[0];
			...
			$link = "http://ims.hdplayers.net/mini/php/yt_link.php?file=".$link;    
    	echo '<item>';
