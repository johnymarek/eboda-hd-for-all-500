...
//http://www.filmeonlinegratis.me/page/2/
if($page) {
    if($search) {
        $html = file_get_contents("http://www.tube8.com/search.html?q=".$search."&page=".$page);
    } else {
        $html = file_get_contents("http://www.filmehdonline.net/page/".$page."/");
    }
} else {
    $page = 1;
    if($search) {
        ...
        $html = file_get_contents("http://www.filmehdonline.net/");
    }
}
...
$videos = explode('<div class="comment" id="post-', $html);
...
foreach($videos as $video) {
    ...
    $t1 = explode('title="Comment on ', $video);
    $t2 = explode('"', $t1[1]);
    $title = $t2[0];
    
    $data = str_between($video,'<div class="time">','</div>');
