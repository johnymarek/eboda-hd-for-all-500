...
class HdboxTemplate {
    ...
    public function generateView($category, $title = null) {
        ...
        switch($category) {
            case HdboxTemplate::VIEW_HOME;
                $this->getHomeHeader();
                break;
            case HdboxTemplate::VIEW_CATEGORY;
                $this->getCategoryHeader();
                break;
            case HdboxTemplate::VIEW_PAGE_NUMBERS:
                $this->getPageNumbersHeader();
                break;

            case HdboxTemplate::VIEW_MOVIE;
                $this->getMovieHeader();
                break;
            case HdboxTemplate::VIEW_MOVIE_DETAIL;
                $this->getMovieDetailHeader();
                break;


            case HdboxTemplate::VIEW_PLAY;
                $this->getPlayHeader();
                break;
        }
        ...
        foreach ($this->mediaItems as $value) {
            $this->getMediaLink($value, $this->movieTitle, $i  ) . "\n";
            ++$i;
        }
        if($this->search) {
...
<mediaDisplay name="photoView"
    ...
    private function getCategoryHeader() {
        ?>
<mediaDisplay name="photoView"
              showHeader="yes" rowCount="5" columnCount="3" drawItemText="no" showDefaultInfo="no"
    ...
    <text redraw="no"
          backgroundColor="-1:-1:-1" foregroundColor="255:255:255"
          offsetXPC="82" offsetYPC="3.1" widthPC="18" heightPC="3" fontSize="10" lines="1">
        <![CDATA[<?php echo resourceString("header_menu_add_bookmark"); ?>]]>
    </text>
...
<mediaDisplay name="threePartsView"
    ...
    <image redraw="no" offsetXPC="2.8" offsetYPC="23.8" widthPC="18.9" heightPC="48.1" backgroundColor="-1:-1:-1" >
                <?php echo SCRAPER_URL."image.php?getimage=" . utf8_encode($this->image); ?>
    </image>
...
<?php
if ( isset( $_GET["downloadmsg"] )){ 
?>
    <!-- DOWNLOAD MESSAGE -->
    <text redraw="no"
          backgroundColor="-1:-1:-1" foregroundColor="0:205:0"
          offsetXPC="23" offsetYPC="12" widthPC="50" heightPC="5" fontSize="16" lines="1">
                      <?php echo $_GET["downloadmsg"]; ?>
    </text>
<?php
}
?>
...
<homePageLink>
    ...
    private function getLink($item,$itemId = null) {
        ...
        if($item[3]) {
            echo "              <media:thumbnail url=\"" .SCRAPER_URL."image.php?getimage=" . utf8_encode($item[3]) . "\"/>\n";
            echo "              <image>" .SCRAPER_URL."image.php?getimage=" . utf8_encode($item[3]) . "</image>\n";
        }
        echo "          </item>\n";
    }
    ...
    //-------------------------------------------------------------------------
    //-------------------------------------------------------------------------
    private function getMediaLink($item, $title, $itemId = null) {
        echo "          <item>\n";
        echo "              <title><![CDATA[" . utf8_encode($item[0]) . "]]></title>\n";
        if($item[1]) {
            echo "              <description><![CDATA[" . utf8_encode($item[1]) . "]]></description>\n";
        ...
        echo "          </item>\n";
        echo "          <item>\n";
        echo "              <title><![CDATA[DOWNLOAD " . utf8_encode($title) . "]]></title>\n";
        if($item[1]) {
            echo "              <description><![CDATA[" . utf8_encode($item[1]) . "]]></description>\n";
        }
        echo "              <link>".SCRAPER_URL."index.php?download&amp;link=". urlencode($item[2]) . "&amp;title=". urlencode($title)."</link>\n";
        if($item[3]) {
            echo "              <media:thumbnail url=\"" .SCRAPER_URL."image.php?getimage=" . utf8_encode($item[3]) . "\"/>\n";
        }
        echo "              <itemid>" . $itemId . "</itemid>\n";
    ...
    private function getSearchLink($item,$itemid) {
        ...
        echo "              <itemid>" . $itemid . "</itemid>\n";
        echo "              <media:thumbnail url=\""  .SCRAPER_URL."image.php?getimage=" . $item[4] . "\"/>\n";
        echo "          </item>\n";
    }
